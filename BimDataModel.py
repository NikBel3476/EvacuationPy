...
def mapping_building(file_buildingjson: str) -> BBuilding:
            ...
            for element in level['BuildElement']:
                ...
                try:
                    build_element = BBuildElement(
                        id=UUID(
                            element['Id']), sign=_sign, name=element['Name'], sizeZ=_sizeZ, output=[
                            UUID(uuid) for uuid in element['Output']], points=[
                            BPoint(
                                p['x'], p['y'], level['ZLevel']) for p in element['XY'][0]['points']])
    ...
    if len(bad_elements) > 0:
        ...
        print(
            f">UnknownException[{__file__}:{frame.f_lineno if not (frame is None) else ()}]. Please check elements from list bellow:")
