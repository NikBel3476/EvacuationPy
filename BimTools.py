...
class Bim:
    ...
    def __init__(self, bim: BBuilding) -> None:
        ...
        if len(incorrect_transits) > 0:
            ...
            print(
                f">TransitGeometryException[{__file__}:{frame.f_lineno if not (frame is None) else ()}]. Please check next transits:")
...
class Zone(BBuildElement):
    ...
    def _calculate_area(self):
        ...
        self._area = round(sum(triangle_area(self._tri.points[tr[0]],
                                             self._tri.points[tr[1]],
                                             self._tri.points[tr[2]]) for tr in self._tri.simplices),
                           NDIGITS)
...
if __name__ == "__main__":
    ...
    t1: Transit = Transit(
        list(
            filter(
                lambda be: be.id == UUID("f46361f4-a99f-4e79-aebe-d36ebed45992"),
                building.levels[0].elements))[0])
