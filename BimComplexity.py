...
class BimComplexity(object):
    ...
    def _calulate(self):
        ...
        if len(visited_zones) != len(self.bim.zones.values()) - 1:
            print(">GraphError: Connectivity on the graph is broken. Zones below is unreachable:")
            for z in filter(lambda x: not x.is_visited and not (x.name == "Safety zone"), self.bim.zones.values()):
                print(f"{z.sign.name}({z.id}, {z.name}) on level at {z.level}")
            
            print('>Use this autogenerated expression for find unreachable zones:')
